...
visual_template_bp = Blueprint('visual_templates', __name__)
...
def create_visual_template():
    ...
    if not data or 'title' not in data or 'layout_data' not in data:
        return jsonify({"msg": "Missing title or layout_data"}), 400
    ...
    return jsonify({"msg": "Visual template created successfully", "template_id": new_template.id}), 201
...
def update_visual_template(template_id):
    ...
    if template.user_id != user_id or template.is_public:
        return jsonify({"msg": "Permission denied"}), 403
    
    if template.layout_style != 'visual':
        return jsonify({"msg": "Cannot update this template type via this endpoint"}), 400
    ...
    db.session.commit()
    return jsonify({"msg": "Visual template updated successfully"}), 200